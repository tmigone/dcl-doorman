...
const bell_01 = new Entity('bell')
engine.addEntity(bell_01)
bell_01.setParent(scene)
...
bell_01.addComponentOrReplace(transform35)
bell_01.addComponent(new GLTFShape('models/Bell_01/Bell.glb'))
bell_01.addComponent(new OnPointerDown(async (e) => {
  log("myEntity was clicked", e)
  let response = await axios.get('https://reqres.in/api/users')
  log(response)
}, {
  hoverText: "Invoke LAND owner (maybe)"
}))